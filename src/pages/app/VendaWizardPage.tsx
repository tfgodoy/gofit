...
import type { StudentCardMasked } from "@/services/gofit-pay/GoFitPayService";
...
export default function VendaWizardPage() {
  ...
  // Cartão principal tokenizado do aluno (gate da venda no cartão)
  ...
  // Verifica o cartão principal do aluno ao escolher pagamento no cartão
  const refreshDefaultCard = useCallback(async () => {
    if (!studentId) return;
    setCardCheckLoading(true);
    const res = await GoFitPayService.listStudentCards(studentId);
    setDefaultCard(
      res.success && res.data
        ? res.data.cards.find(c => c.is_default && c.status === "active") ?? null
        : null
    );
  ...
  useEffect(() => {
    if (formaPagamento === "cartao_credito") refreshDefaultCard();
  }, [formaPagamento, refreshDefaultCard]);
      ...
      <AppLayout>
        <div className="px-8 py-6 max-w-2xl space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            {/* Gate do cartão: venda no cartão exige cartão principal cadastrado */}
            {formaPagamento === "cartao_credito" && (
              cardCheckLoading ? (
                <div className="mt-4 flex items-center gap-2 text-sm text-gray-400">
                  <Loader2 className="w-4 h-4 animate-spin" /> Verificando cartão cadastrado...
                </div>
              ) : defaultCard ? (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-sm font-bold text-green-700">
                        Cobrança automática no cartão principal: {defaultCard.card_brand} **** {defaultCard.card_last4}
                      </p>
                      <p className="text-xs text-green-600">A 1ª parcela será cobrada ao concluir a venda.</p>
                    </div>
                  </div>
                  <button onClick={() => setShowCardsModal(true)}
                    className="text-xs font-semibold text-green-700 hover:underline whitespace-nowrap">
                    Gerenciar
                  </button>
                </div>
              ) : (
                <div className="mt-4 bg-orange-50 border border-orange-200 rounded-xl px-4 py-3 space-y-2">
                  <p className="text-sm font-bold text-orange-700">
                    O aluno não possui cartão principal cadastrado.
                  </p>
                  <p className="text-xs text-orange-600">
                    Para vender no cartão com cobrança automática, cadastre o cartão agora
                    (ou gere o link para o aluno cadastrar pelo celular).
                  </p>
                  <button
                  ...
                  >
                    <CreditCard className="w-3.5 h-3.5" /> CADASTRAR CARTÃO
                  </button>
                </div>
              )
            )}
          ...
          <div className="flex justify-end gap-3 pb-8">
            ...
            <button
              ...
              disabled={!formaPagamento || saving || (formaPagamento === "cartao_credito" && (cardCheckLoading || !defaultCard))}
        ...
        {/* Carteira de cartões — cadastrar/gerenciar sem sair da venda */}
        {showCardsModal && student && (
          <StudentCardsModal
            studentId={studentId!}
            studentName={student.nome_completo}
            companyName={user?.contractorName ?? "sua academia"}
            onClose={() => { setShowCardsModal(false); refreshDefaultCard(); }}
          />
        )}
      </AppLayout>
    );
