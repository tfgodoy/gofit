...
// ── dados mockados para os gráficos (histórico) ────────────
const mrrHistory = [
...
// ── sidebar nav ────────────────────────────────────────────
const navItems = [
  { icon: BarChart2, label: "Dashboard",      to: "/owner/dashboard" },
  { icon: Building2, label: "Empresas",       to: "/owner/empresas" },
  { icon: Users,     label: "Usuários",       to: "/owner/usuarios" },
  { icon: CreditCard,label: "Financeiro",     to: "/owner/financeiro" },
  { icon: Settings,  label: "Configurações",  to: "/owner/configuracoes" },
];
...
export default function OwnerDashboard() {
  ...
  const [stats, setStats] = useState<Stats>({
    mrr: 0, mrrGrowth: 0, activeCompanies: 0,
    newThisMonth: 0, trialCompanies: 0, churnThisMonth: 0,
    totalStudents: 0, avgTicket: 0,
  });
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <a href="/cadastro" className="text-xs font-semibold text-primary hover:underline">Cadastrar primeira empresa →</a>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-gray-50">
                  {companies.map(c => (
                    <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3.5">
                        <button className="text-xs text-primary font-semibold hover:underline">Detalhes</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
